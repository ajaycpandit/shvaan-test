...
function buildInv(b) {
  ...
  const rows=entries.map(e=>{
    ...
    // Itemized extra charges. New bookings carry e.lines (one per matched rule);
    // older bookings only have e.surcharge, so fall back to a single line.
    let sr='';
    if(e.lines && e.lines.length){
      sr=e.lines.map(l=>`<tr><td style="padding-left:10px;color:var(--ink-faint);font-size:10px">↳ ${esc(l.label||'Late checkout')}</td><td colspan="2"></td><td style="text-align:right;color:var(--ink-faint);font-size:10px">$${parseFloat(l.amount).toFixed(2)}</td></tr>`).join('');
    } else if(e.surcharge>0){
      sr=`<tr><td style="padding-left:10px;color:var(--ink-faint);font-size:10px">↳ Late checkout</td><td colspan="2"></td><td style="text-align:right;color:var(--ink-faint);font-size:10px">$${parseFloat(e.surcharge).toFixed(2)}</td></tr>`;
    }
    return `<tr><td><strong>${esc(e.dogName||'')}</strong></td><td>$${parseFloat(e.rate).toFixed(2)}/day</td><td>${dl}</td><td>$${(parseFloat(e.total)-parseFloat(e.surcharge||0)).toFixed(2)}</td></tr>${sr}`;
  }).join('');
  return `
    <div class="iheader">
