...
async function saveCio(){
  ...
  try{
    if(cioMode==='in'){
      ...
      // Offer to clean up stay photos now (only if any exist). If declined, they remain
      // and the reservation is flagged in the "Photos to review" reminder.
      if(typeof listStayPhotos==='function'){
        try{
          const pics = await listStayPhotos(r.id);
          if(pics.length){
            if(confirm(pics.length+' stay photo'+(pics.length!==1?'s':'')+' for '+(r.dog_name||'this dog')+'.\n\nDelete them now? (Choose Cancel to keep them — you can delete later from the Photos button, and they\'ll show in the reminder list.)')){
              try{ await deleteStayPhotos(r.id); toast('Stay photos deleted.'); }
              catch(delErr){ toast('Could not delete photos: '+delErr.message, true); }
            }
